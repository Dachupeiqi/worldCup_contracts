...
const graphUrl = "https://api.studio.thegraph.com/query/46165/worldcup/0.0.4"
...
function getPlayerRewardList(totalReward: string, records: any, winner: number) {
  ...
  const temp=records.map
    (
      (it: { selectCountry: number; }) => {
        // 猜中奖励翻倍
    ...
    (
      (element:
        {
          weight: any;   player: string | number;
        }) => {
...
async function main() {
  ...
  // console.log('请手动发放奖励!\n\n');此处调用WordCupDistribute合约的distributeReward方法
